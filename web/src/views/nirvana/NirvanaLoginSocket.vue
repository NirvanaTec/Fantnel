      ...
      <div class="login-form">
        <form @submit.prevent="toLogin">
          <div class="form-group">
            ...
            <input v-model="account" class="form-input" id="Account" placeholder="请输入您的账号" type="text" required>
          </div>

          <div class="form-group">
            <label class="form-label" for="Password">密码</label>
            <input v-model="password" class="form-input" id="Password" placeholder="请输入您的密码" type="password" required>
          </div>
...
const toLogin = async () => {
  try {
    ...
    if (data.code === 1) {
      const accountData = await getNirvanaAccount();
      if (accountData.code === 1) {
        if (accountData.data.days > 0) {
...
onMounted(() => {
  ...
  getNirvanaAccount().then(data => {
    if (data.code == 1 && data.data.days > 0) {
      // 如果已登录，直接跳转至用户首页
