...
public static class AccountMessage
{
    ...
    public static void Login(EntityAccount account)
    {
        lock (LockManager.LoginLock)
        {
            ...
            if (PublicProgram.Services == null)
                throw new Code.ErrorCodeException(Code.ErrorCode.ServicesNotInitialized);
            ...
            if (account.Type == "cookie")
            {
                ...
                try
                {
                    req = JsonSerializer.Deserialize<EntityX19CookieRequest>(account.Password);
                }
                catch
                {
                    req = new EntityX19CookieRequest { Json = account.Password };
                }

                if (req == null) throw new Code.ErrorCodeException(Code.ErrorCode.Failure);
                // result = await NirvanaPublic.Services.X19.ContinueAsync(account.Password);
                result = PublicProgram.Services.Wpf.LoginWithCookie(req);
            }
            ...
            //             {
            //                 var loginJson =
            //                     await NirvanaPublic.Services.C4399.LoginWithPasswordAsync(account.Account, account.Password);
            //                 result = await NirvanaPublic.Services.X19.ContinueAsync(loginJson);
            //             }
            //             catch (Exception ex) when (ex.Data.Contains("captcha_url") && ex.Data.Contains("session_id"))
            //             {
            //                 var captcha = new Game4399Captcha
            //                 {
            //                     CaptchaUrl = ex.Data["captcha_url"] as string,
            //                     SessionId = ex.Data["session_id"] as string
            //                 };
            //                 Log.Warning("验证码链接: {captchaUrl}", captcha.CaptchaUrl);
            //                 if (captcha.IsEmpty())
            //                     throw new Code.ErrorCodeException(Code.ErrorCode.CaptchaError, captcha);
            //             }
            //
            //             break;
            //         }
